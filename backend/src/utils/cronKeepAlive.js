...
export const initKeepAlive = () => {
  ...
  if (process.env.NODE_ENV === "development" || process.env.NODE_ENV !== "production") {
    console.log("[Keep-Alive] 개발 환경(NODE_ENV !== 'production')이므로 Self-Ping Cron을 실행하지 않습니다.");
    return;
  }

  const FOURTEEN_MINUTES = 14 * 60 * 1000; // 14분 (밀리초)

  console.log("[Keep-Alive] 프로덕션 환경: 14분 주기 서버 Self-Ping Cron이 초기화되었습니다.");
  ...
  setInterval(() => {
    ...
    if (!serverUrl) {
      console.log("[Keep-Alive Ping] SERVER_URL 또는 RENDER_EXTERNAL_URL 환경변수가 없어 Self-Ping을 건너뜁니다.");
      return;
    }
    ...
    console.log(`[Keep-Alive Ping] ${new Date().toLocaleString("ko-KR")} - Self-Ping 요청 전송: ${healthUrl}`);

    protocol
      .get(healthUrl, (res) => {
        console.log(`[Keep-Alive Ping Success] 상태 코드: ${res.statusCode}`);
      })
      .on("error", (err) => {
